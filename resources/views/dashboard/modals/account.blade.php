    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title pull-left" id="accountModalLabel">اضافة حساب</h5>
...
<script>
    $('.account').click(function () {
        if ($(this).hasClass("update")) {
            ...
            $('#accountModal .form').text('تعديل حساب')
            $('#accountModal .form').append('<input type="hidden" name="_method" value="PUT">')

            //set data to inputs
            $('#accountModal input[name="amount"]').val($(this).data('amount'))
        }else {
            $('#accountModal .form').attr('action', "{{ route('accounts.store') }}")
            $('#accountModal .modal-title').text('إضافة حساب')

            $('#accountModal input[name="amount"]').val('')
        }
