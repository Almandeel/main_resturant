                ...
                <tbody>
                    @foreach ($accounts as $account)
                        <tr>
                            ...
                            <td>{{ number_format($account->fromEntries()->sum('amount') ?? 0, 2) }}</td>
                                ...
                                <div class="btn-group">
                                    ...
                                    {{-- @if($account->balance() <= 0)
                                        @permission('accounts-update')
                                            <button class="btn btn-warning btn-xs account update" data-action="{{ route('accounts.update', $account->id) }}" data-amount="{{ $account->balance() }}" ><i class="fa fa-edit"></i> تعديل </button>
                                        @endpermission
                                    @else  --}}
                                        @permission('accounts-create')
                                            <button class="btn btn-primary btn-xs account" ><i class="fa fa-plus"></i> اضافة </button>
                                        @endpermission
                                    {{-- @endif --}}
                                </div>
                            </td>
